...
describe('+ readFile() **Promise**', function () {
  var TEST_DIR
  ...
  it('should read and parse JSON', function (done) {
    ...
    jf.readFile(file)
      .then((obj2) => assert.equal(obj2.name, obj.name))
  ...
  describe('> when invalid JSON', function () {
    it('should include the filename in the error', function (done) {
      ...
      jf.readFile(file)
        .then((obj2) => assert.strictEqual(obj2, undefined))
        .catch((err) => {
          assert(err instanceof Error)
          assert(err.message.match(fn))
        })
  ...
  describe('> when invalid JSON and throws set to true', function () {
    it('should return an error', function (done) {
      ...
      jf.readFile(file)
        .then((obj2) => assert.strictEqual(obj2, null))
        .catch((err) => {
          assert(err instanceof Error)
          assert(err.message.match(fn))
        })
  ...
  describe('> when passing null and not callback', function () {
    it('should not throw an error', function (done) {
      var file = path.join(TEST_DIR, 'somefile.json')
      ...
      jf.readFile(file, null)
        .then((obj2) => assert.strictEqual(obj.name, 'jp'))
        .catch((err) => assert.ifError(err))
        .then(done)
    })
  })
  ...
  describe('> w/ BOM', function () {
    it('should properly parse', function (done) {
      ...
      fs.writeFileSync(file, '\uFEFF' + JSON.stringify(obj))
      jf.readFile(file)
        .then((data) => assert.deepEqual(obj, data))
        .catch((err) => assert.ifError(err))
        .then(done)
    })
  })
